# Windows 11 24H2 Update Blocker - Windows PowerShell 5.1 Compatible Version
# Blocks Windows 11 24H2 feature update with multiple protection levels
# Compatible with Windows PowerShell 5.1 (ships with Windows)
...
$script:ScriptPath = "$env:ProgramData\ftfy-no-means-no\Block-24H2-v5.ps1"
$script:LogFile = "$script:LogPath\Block-24H2-$(Get-Date -Format 'yyyy-MM').log"

# Help text
if ($Help) {
    Write-Host @"
Block-24H2-v5.ps1 - Windows 11 24H2 Update Blocker (Windows PowerShell 5.1 Version)

USAGE:
    .\Block-24H2-v5.ps1 [-Mode <mode>] [-Silent] [-WhatIf] [-ScheduleTask] [-Schedule <schedule>]
...
EXAMPLES:
    ...
    .\Block-24H2-v5.ps1 -Mode Selective -Silent
    
    # Test what enhanced mode would do
    .\Block-24H2-v5.ps1 -Mode Enhanced -WhatIf
    
    # Schedule daily selective blocking
    .\Block-24H2-v5.ps1 -Mode Selective -ScheduleTask -Schedule Daily
...
function Show-AdminInstructions {
    ...
    Write-Host "Method 2: Use Windows Terminal as Admin" -ForegroundColor Cyan
    Write-Host "Method 3: Run from an elevated Command Prompt:" -ForegroundColor Cyan
    Write-Host "          powershell -ExecutionPolicy Bypass -File Block-24H2-v5.ps1" -ForegroundColor Green
    Write-Host ""
    Write-Host "=================================================================" -ForegroundColor Red
}
...
function Copy-ScriptToPersistentLocation {
    ...
    if ($PSCommandPath) {
        ...
        $currentScriptPath = Join-Path (Get-Location) "Block-24H2-v5.ps1"
...
function Request-ScheduledTaskInstallation {
    ...
    if ($response -eq "Y") {
        ...
        if (New-ScheduledBlockTask -SelectedMode $SelectedMode -ScriptLocation $persistentScript) {
            ...
            Write-Host ""
            Write-Host "[X] Failed to install scheduled task" -ForegroundColor Red
        }
    } else {
        Write-Host ""
...
function Show-Menu {
    ...
    Write-Host "|          Windows 11 24H2 Update Blocker (v5.1)            |" -ForegroundColor Cyan
    Write-Host "+============================================================+" -ForegroundColor Cyan
    Write-Host ""
...
function Get-BlockStatus {
    ...
    if (Test-Path $regPath) {
        ...
        if ($targetRelease.TargetReleaseVersionInfo) {
            Write-Host "[OK] Target Release: " -NoNewline -ForegroundColor Green
            Write-Host "$($targetRelease.TargetReleaseVersionInfo)" -ForegroundColor White
            $status.TargetRelease = $true
        } else {
            Write-Host "[X] Target Release: " -NoNewline -ForegroundColor Red
        ...
        if ($deferral.DeferFeatureUpdates -eq 1) {
            Write-Host "[OK] Feature Updates: " -NoNewline -ForegroundColor Green
            Write-Host "Deferred" -ForegroundColor White
            $status.Deferred = $true
        } else {
            Write-Host "[X] Feature Updates: " -NoNewline -ForegroundColor Yellow
            Write-Host "Not deferred" -ForegroundColor Gray
            $status.Deferred = $false
        }
    } else {
        Write-Host "[X] Windows Update Policy: " -NoNewline -ForegroundColor Red
        Write-Host "Not configured" -ForegroundColor Gray
    ...
    if ($wuService) {
        if ($wuService.Status -eq "Running" -or $wuService.StartType -ne "Disabled") {
            Write-Host "[OK] Windows Update Service: " -NoNewline -ForegroundColor Green
...
function Main {
    ...
    try {
        Initialize-Logging
        Write-Log "Block-24H2 script started (Windows PowerShell 5.1 version)" "Info"
    } catch {
